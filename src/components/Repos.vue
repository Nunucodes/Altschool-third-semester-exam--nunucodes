  ...
  <div class="container">
    ...
    <ul class="repo">
      ...
      >
        <router-link
          class="link"
          :to="{ name: 'SingleRepo', params: { id: repo.id } }"
        >
          <h2>{{ repo.name }}</h2>
          ...
          <div class="lang-size">
            <p className="repo-lang">
              <i class="fa-regular fa-circle-dot"></i> {{ repo.language }}
            </p>
    ...
    async fetchRepositories() {
      ...
      try {
        const response = await axios.get(
          "https://api.github.com/users/Nunucodes/repos"
        );
...
.container {
  ...
  .btn-container {
    ...
    button {
      ...
      &:hover {
        background: rgba(110, 211, 135, 0.5);
        color: $text-color;
      }
    }
